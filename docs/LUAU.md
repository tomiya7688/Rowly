...
すべての実行 API は、明示的な標準機能の許可リストと read-only の `Rowly` API を持つサンドボックスで動作する。任意ファイル、ネットワーク、OS コマンド、外部プロセス、ネイティブライブラリへのアクセスは公開せず、1.0 では解除設定を提供しない。

標準ライブラリの公開範囲、`require` / `loadstring` / `print` 等を公開しない互換性変更、変数の実行間分離、保護の保証範囲は [LUAU_SANDBOX.md](LUAU_SANDBOX.md) を正本とする。
...
公開 API は、GUI や Rowly DSL と同様に `process` 境界を越えない形で追加する。保存・モジュール等は将来候補であり、1.0 のサンドボックスに任意ファイルアクセスや解除手段を追加するものではない。
